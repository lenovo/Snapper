...
//snapper: exception code
#define ERROR_MESSAGE_NOT_RELATIVE_URI() (U("not relative uri")),ERROR_NOT_RELATIVE_URI
#define ERROR_MESSAGE_UNSUPPORTED_SEGMENT(IDENTIFIER) ((IDENTIFIER) + U(" unsupported")),ERROR_UNSUPPORTED_SEGMENT
#define ERROR_MESSAGE_RESOURCE_NOT_FOUND(IDENTIFIER) (U("resource ") + (IDENTIFIER) + U(" not found")),ERROR_RESOURCE_NOT_FOUND
#define ERROR_MESSAGE_UNEXPECTED_PATH_ROOT(IDENTIFIER) (U("unexpected ") + (IDENTIFIER) + U(" on path root")),ERROR_UNEXPECTED_PATH_ROOT
#define ERROR_MESSAGE_UNEXPECTED_PARENTHESIS_EXPRESSION(IDENTIFIER) ((IDENTIFIER) + U(" cannot have parenthesis expression")),ERROR_UNEXPECTED_PARENTHESIS_EXPRESSION
#define ERROR_MESSAGE_EMPTY_SEGMENT_IDENTIFIER() (U("empty segment identifier")),ERROR_EMPTY_SEGMENT_IDENTIFIER
#define ERROR_MESSAGE_PARENTHESIS_MISMATCH() (U("parenthesis mismatch")),ERROR_PARENTHESIS_MISMATCH
#define ERROR_MESSAGE_NAME_MUST_BE_PROVIDED() (U("name must be provided for each value")),ERROR_NAME_MUST_BE_PROVIDED
#define ERROR_MESSAGE_NAME_MUST_NOT_DUPLICATE(NAME) (U("name ") + (NAME) + U(" must not duplicate")),ERROR_NAME_MUST_NOT_DUPLICATE
#define ERROR_MESSAGE_KEY_COUNT_MISMATCH() (U("key count mismatch")),ERROR_KEY_COUNT_MISMATCH
#define ERROR_MESSAGE_KEY_NOT_FOUND(NAME) (U("key name ") + (NAME) + U(" not found")),ERROR_KEY_NOT_FOUND
#define ERROR_MESSAGE_CLOSE_CHAR_EXPECTED(CHAR) (::odata::utility::string_t(U("close char ")) + (CHAR) + U(" expected")),ERROR_CLOSE_CHAR_EXPECTED
#define ERROR_MESSAGE_NOT_OPEN_TYPE(NAME) (U("dynamic property ") +(NAME) + U(" under nonopen type")),ERROR_NOT_OPEN_TYPE
#define ERROR_MESSAGE_NO_NAVIGATION_SOURCE() (U("no navigation source for navigation property")),ERROR_NO_NAVIGATION_SOURCE
#define ERROR_MESSAGE_PARAMETER_COUNT_MISMATCH() (U("parameter count mismatch")),ERROR_PARAMETER_COUNT_MISMATCH
#define ERROR_MESSAGE_PARAMETER_NOT_FOUND(NAME) (U("parameter name ") + (NAME) + U(" not found")),ERROR_PARAMETER_NOT_FOUND
#define ERROR_MESSAGE_BOUND_OPERATION_IMPORT_FOUND(NAME) (U("bound operation import ") + (NAME) + U(" found")),ERROR_BOUND_OPERATION_IMPORT_FOUND
#define ERROR_MESSAGE_UNBOUND_OPERATION_FOUND(NAME) (U("unbound operation ") + (NAME) + U(" found")),ERROR_UNBOUND_OPERATION_FOUND
#define ERROR_MESSAGE_BINDING_PARAMETER_NOT_FOUND(NAME) (U("binding parameter for ") + (NAME) + U(" not found")),ERROR_BINDING_PARAMETER_NOT_FOUND
#define ERROR_MESSAGE_BINDING_TYPE_MISMATCH(NAME) (U("binding type for ") + (NAME) + U(" mismatch")),ERROR_BINDING_TYPE_MISMATCH
#define ERROR_MESSAGE_SEGMENT_FOLLOW(NAME) (::odata::utility::string_t(U("segment follow ")) + (NAME)),ERROR_SEGMENT_FOLLOW
#define ERROR_MESSAGE_SEGMENT_FOLLOW_NONCOMPOSABLE(NAME) (U("segment follow noncomposable operation (import) ") + (NAME)),ERROR_SEGMENT_FOLLOW_NONCOMPOSABLE
#define ERROR_MESSAGE_INVALID_COUNT_QUERY(QUERY) (U("invalid count query ") + (QUERY)),ERROR_INVALID_COUNT_QUERY
#define ERROR_MESSAGE_INVALID_CHARACTER(CHAR) (::odata::utility::string_t(U("invalid character ")) + (CHAR)),ERROR_INVALID_CHARACTER
#define ERROR_MESSAGE_UNEXPECTED_TOKEN(NAME) (U("unexpected token ") + (NAME)),ERROR_UNEXPECTED_TOKEN
#define ERROR_MESSAGE_TOKEN_KIND_EXPECTED(KIND) (::odata::utility::string_t(U("token kind expected ")) + (KIND)),ERROR_TOKEN_KIND_EXPECTED
#define ERROR_MESSAGE_CHAR_KIND_EXPECTED(KIND) (::odata::utility::string_t(U("char kind expected ")) + (KIND)),ERROR_CHAR_KIND_EXPECTED
#define ERROR_MESSAGE_INVALID_VALUE(VALUE) (U("invalid value ") + VALUE),ERROR_INVALID_VALUE
#define ERROR_MESSAGE_EMPTY_NAME_VALUE_PAIR() (U("empty name-value pair")),ERROR_EMPTY_NAME_VALUE_PAIR
#define ERROR_MESSAGE_DUPLICATE_RANGE_VARIABLE(NAME) (U("duplicate range variable ") + (NAME)),ERROR_DUPLICATE_RANGE_VARIABLE
#define ERROR_MESSAGE_FILTER_ON_NONCOLLECTION() (U("$filter on non-collection resource")),ERROR_FILTER_ON_NONCOLLECTION
#define ERROR_MESSAGE_ORDERBY_ON_NONCOLLECTION() (U("$orderby on non-collection resource")),ERROR_ORDERBY_ON_NONCOLLECTION
#define ERROR_MESSAGE_SELECT_OR_EXPAND_ON_NONSTRUCTURED_TYPE() (U("$select or $expand on non-structured type")),ERROR_SELECT_OR_EXPAND_ON_NONSTRUCTURED_TYPE
...
#define DBG_PREFIX   0   //enable/disable filename:line:function
#define DEBUG_X_ON

#ifdef DEBUG_X_ON
#define ODATA_DEBUG_X(x) (DBG_PREFIX ? ODATA_TRACE_X(x):ODATA_TRACE_Y(x))
#else
...
void odata_path_parser::bind_path(std::vector<::odata::utility::string_t> raw_segments)
{
	...
	{
        ODATA_DEBUG_X("-------------" << std::endl << "!!!raw segments " << "'" << raw_segments[i] << "'" << std::endl);
		if (i == 0)
		{
        ...
        for (::size_t i = 0; i < m_bound_segments.size(); ++i)
        {
            ODATA_DEBUG_X("!!!odata_path_parser::bind_path 1: m_bound_segments index[" << i << "] type = " << m_bound_segments[i]->segment_type()  << std::endl);
...
void odata_path_parser::bind_first_segment(const ::odata::utility::string_t &segment_str)
{
    ...
    ODATA_DEBUG_X("bind_first_segment string: '" << segment_str << "' -> " << identifier << " | (" << parenthesis_exp  << ")" << std::endl);
	...
	{
        ODATA_DEBUG_V("try_bind_as_navigation_source");             
		return;
	}
	else if (try_bind_as_operation_import(identifier, parenthesis_exp))
	{
        ODATA_DEBUG_V("try_bind_as_operation_import");             
		return;
	}
...
void odata_path_parser::bind_next_segment(const ::odata::utility::string_t &segment_str)
{
    ...
    ::odata::utility::string_t parenthesis_exp;
    extract_segment_identifier_and_parenthesis_exp(segment_str, identifier, parenthesis_exp);
    ODATA_DEBUG_X("bind_next_segment string: '" << segment_str << "' -> " << identifier << " | (" << parenthesis_exp  << ")" << std::endl);//snapper print
    ...
    //snapper
    //after try bind as type, try key bind for entity set(not snapper) or singleton or type(ResourceCollection)
    if (try_bind_as_key_of_resource_collection(identifier, parenthesis_exp))
    {
        return;
    }  
    ...
    // snapper 
    // for exception urls
    if(try_bind_as_exception(identifier, parenthesis_exp))
    {
        return;
    }
	//snapper: stop here
	throw odata_exception("no available binding for "+identifier, ERROR_NO_AVAILABLE_BINDING);

	bind_as_dynamic_property(identifier, parenthesis_exp);
    ODATA_DEBUG_X("bind nothing for " << identifier << std::endl);
}
...
{
  auto containers = model()->find_all_containers();//several serviceroot namespaces have same container name "ServiceContainer"
  if (containers.size() == 0)
    return false;

  for(int j=0;j<containers.size();j++){
    auto container = containers[j];
    //ODATA_DEBUG_X(" container " << container->get_name() << std::endl);

	auto entity_set = container->find_entity_set(identifier);
	if (entity_set != nullptr)//snapper: RF--EntitySet is for top-level Resource Collections
	{
        ...
        ODATA_DEBUG_X("=>create entity set seg for " << identifier << std::endl);
        bind_as_key(key_exp);
	...
	else
	{
		...
		if (singleton != nullptr)//snapper: RF--Singleton is for individual resources (including Resource Collections)
		{
			auto singleton_segment = odata_path_segment::create_singleton_segment(singleton);
			m_bound_segments.push_back(singleton_segment);
			ODATA_DEBUG_X("=>create singleton seg for '" << identifier << "' - [" << singleton->get_name() << "]:<" << singleton->get_entity_type_name() << ">" << std::endl);
            ...
            // snapper, create entity type segments for singleton
            if(singleton->get_entity_type() == nullptr)
            {
                throw odata_exception(ERROR_MESSAGE_RESOURCE_NOT_FOUND(identifier));
            }

            try_bind_as_type(singleton->get_entity_type()->get_name(), key_exp);
...
{
	auto segment = previous_segment();
    ODATA_DEBUG_V(segment->segment_type()); 
...
void odata_path_parser::bind_as_key(const ::odata::utility::string_t &key_exp)
{
    ...
    ODATA_DEBUG_X("=>create key segment" << std::endl);
...
bool odata_path_parser::try_bind_Oem_as_declared_property(const ::odata::utility::string_t &identifier, const ::odata::utility::string_t &key_exp)
{
#define OEM_NAME      "Oem"  // Oem complex name
#define LENOVO_NAME   "Lenovo" // to match complex name "Lenovo" in each oem schema

    //target_type()/identifier: Oem/Lenovo, Lenovo/XXXLenovoResc
    //Oem and Lenovo Complex name should not change
    if( target_type()->get_name().compare(OEM_NAME) == 0 && identifier.compare(LENOVO_NAME) == 0 ||
		target_type()->get_name().compare(LENOVO_NAME) == 0)
    {


        std::string parent_entity;
        std::shared_ptr<::odata::core::odata_path_segment> pre_seg = previous_segment();
        if(pre_seg != nullptr && pre_seg->as<odata_type_segment>() != nullptr)
        {
            std::shared_ptr<::odata::edm::edm_named_type> segtype = pre_seg->as<odata_type_segment>()->type();
            parent_entity = segtype->get_name();
            ODATA_DEBUG_X("   (Oem parent entity) " << segtype->get_full_name() << std::endl);
        }

        std::shared_ptr<::odata::edm::edm_complex_type> owning_type;
        if(target_type()->get_name().compare(OEM_NAME) == 0) //  Oem/Lenovo
        {
            ODATA_DEBUG_X("       search complex  " << target_type()->get_name() << " for identifier " << identifier << " in " << parent_entity << std::endl);
            // use parent resource entity name to locate "Oem"
            owning_type = model()->find_complex_type(target_type()->get_name(), identifier, parent_entity);    
        }
        else // Oem/Lenovo/XXX
        {
            ODATA_DEBUG_X("       search complex  " << target_type()->get_name() << " for identifier " << identifier << " (" << parent_entity << ")" << std::endl);
            ODATA_DEBUG_X("           complex Lenovo:  " << target_type()->get_full_name() << ", " << target_type()->get_namespace() << std::endl);
            // user name space of "Lenovo" to locate the complex 
            owning_type = model()->find_complex_type_by_ns(target_type()->get_name(), identifier, target_type()->get_namespace());    
        }

		if (owning_type == nullptr)
		{
            ODATA_DEBUG_X("    no oem complex " << target_type()->get_name() << " containing property " << identifier << std::endl);
			return false;
		}
        ...
        if (property == nullptr)
		{
	        ODATA_DEBUG_X("    complex " << target_type()->get_name() << " not containing " << identifier << std::endl);
			return false;
		}
		ODATA_DEBUG_X("   (Oem) " << owning_type->get_name() << "[" << owning_type->get_base_type_name() << "] contains " << identifier << std::endl);
	    ...
	    if (property->get_property_type()->get_type_kind() == ::odata::edm::edm_type_kind_t::Navigation)
	    {
			...
			{
	            // snapper: redfish doesn't have follow this rule and so do not throw exception
	        ...
	        // snapper, 
	        // if the navigation type name is a entity, then create type segment instead
	        ODATA_DEBUG_V(navigation_type->get_navigation_type()->get_name());        
	        auto entity_type = model()->find_entity_type(navigation_type->get_navigation_type()->get_name());
	        if (entity_type)
	        {
	            try_bind_as_type(entity_type->get_name(), key_exp, true);
	        }
	        else//collection
	        {
	            m_bound_segments.push_back(odata_path_segment::create_navigation_property_segment(owning_type, property, navigation_type));
	            ODATA_DEBUG_X("=>create oem navigationprop seg for '" << identifier <<"' [" << navigation_type->get_name() << "] " << std::endl);
	            bind_as_key(key_exp);
	        }
	    }
	    else
	    {
		    ...
		    ODATA_DEBUG_X("=>create oem struc prop seg  '" << identifier << "' [" << target_type()->get_name() << "] : <" << property->get_property_type()->get_name()<< ">" << std::endl);
...
bool odata_path_parser::try_bind_as_exception(const ::odata::utility::string_t &identifier, const ::odata::utility::string_t &key_exp)
{
    if((target_type()->get_name() == "Bios") && (identifier == "Pending"))
    {
        return true;
    }
...
{
    // snapper: same entity type name can be found in different schemas
    // use both entity type name and property name for search
    ODATA_DEBUG;
	auto owning_type = model()->find_entity_type(target_type()->get_name(), identifier);    
    if(owning_type == nullptr)
    {
        ODATA_DEBUG_X("    no entity type " << target_type()->get_name() << " containing property " << identifier << std::endl);
		return try_bind_Oem_as_declared_property(identifier, key_exp);        
    }

    auto property = owning_type->find_property(identifier);
    if (property == nullptr)
    {
        ODATA_DEBUG_X("    " << target_type()->get_name() << " not found property " << identifier << std::endl);
        return false;
    }
    ODATA_DEBUG_X("    " << target_type()->get_name() << " found property for '" << identifier << "':<" << property->get_property_type()->get_name()<< ">" << std::endl);
    ...
    if (property->get_property_type()->get_type_kind() == ::odata::edm::edm_type_kind_t::Navigation)
    {
        ...
        {
            m_bound_segments.push_back(odata_path_segment::create_navigation_property_segment(owning_type, property, navigation_type));
            ODATA_DEBUG_X("=>create navigationprop seg for '" << identifier <<"' [" << navigation_type->get_name() << "] " << std::endl);
            bind_as_key(key_exp);
        }
    }
    else
    {
        // Structural property.
        m_bound_segments.push_back(odata_path_segment::create_structural_property_segment(owning_type, property));
        ODATA_DEBUG_X("=>create struc prop seg  '" << identifier << "' [" << target_type()->get_name() << "] : <" << property->get_property_type()->get_name()<< ">" << std::endl);
        // Structural property cannot have key expression.
        if (!key_exp.empty())
        {
            throw odata_exception(ERROR_MESSAGE_UNEXPECTED_PARENTHESIS_EXPRESSION(identifier));
...
bool odata_path_parser::try_bind_as_type(const ::odata::utility::string_t &identifier, const ::odata::utility::string_t &empty_exp, bool ignore_prop_check)
{
    // check if the entity type is a valid property of previous segment, 
    // this is to prevent invalide url like Systems/Bios, AccountService/Bios
    // but we need to ignore the check if the validation has been done otherwhere
    auto segment = previous_segment();
    if (segment && !ignore_prop_check && (target_type() != nullptr))
        //&&((segment->segment_type() == ::odata::core::odata_path_segment_type::Type)
        //|| (segment->segment_type() == ::odata::core::odata_path_segment_type::Key)))
    {
        auto target_entity_type = ::odata::edm::edm_model_utility::get_entity_type(target_type());
        auto owning_type = model()->find_entity_type(target_type()->get_name(), identifier); 
        if(owning_type == nullptr)
        {
            ODATA_DEBUG_X("try_bind_as_type for '" << identifier << "', not a property of resource " << target_type()->get_name() << std::endl);
            return false;
        }
    }
    ...
    if (type != nullptr)
		basename = model()->find_entity_type(identifier)->get_base_type_name();
    ...
    if (type == nullptr)
    {
        ...
        {
            // Not a type segment.
			ODATA_DEBUG_X("    try_bind_as_type failed for " << identifier << std::endl);
            return false;
        }
		basename = model()->find_complex_type(identifier)->get_base_type_name();
    }
	...
	ODATA_DEBUG_X("=>create type seg for '" << identifier << "' - [" << type->get_name() << "]:<" << basename << "> edm_type_kind_t:"<< type->get_type_kind()<< std::endl);

    return true;
}

//snapper
//support key of entity set or singleton:   
//    E.g. Systems/1/Netxxxx, Managers/1/Ethernetinterface/2/xxx
bool odata_path_parser::try_bind_as_key_of_collection(const ::odata::utility::string_t &identifier, const ::odata::utility::string_t &empty_exp)
{
    ODATA_DEBUG;
	auto segment = previous_segment();
    if (segment->segment_type() != ::odata::core::odata_path_segment_type::Singleton 
        && segment->segment_type() != ::odata::core::odata_path_segment_type::EntitySet )
    {
        return false;
	}

    if (!empty_exp.empty()) // key of collection does not need parenthesis_exp
    {
        throw odata_exception(ERROR_MESSAGE_UNEXPECTED_PARENTHESIS_EXPRESSION(identifier));
    }
	...
	compute_target_navigation_source_and_type();
	auto target_entity_type = ::odata::edm::edm_model_utility::get_entity_type(target_type());
    ODATA_DEBUG_V(target_entity_type->get_base_type()->get_type_kind());

    auto keys = parse_named_values(identifier); // needs ''
	auto keys_from_model = target_entity_type->get_key_with_parents(); 

	if (keys_from_model.size() != keys.size() || keys.size() != 1)  // check model definition of key - Resource_v1.xml in redfish metadata: <PropertyRef Name="MemberId"/>
	{
		throw odata_exception(ERROR_MESSAGE_KEY_COUNT_MISMATCH());
	}
    ...
    ODATA_DEBUG_X("=>collection key name/value: " << keys[0].first << "/" << keys[0].second->to_string() << " .model key: " << keys_from_model[0] << std::endl);
	m_bound_segments.push_back(odata_path_segment::create_key_segment(target_navigation_source(), target_entity_type, std::move(keys)));
    
    return true;
}

bool odata_path_parser::try_bind_as_key_of_resource_collection(const ::odata::utility::string_t &identifier, const ::odata::utility::string_t &empty_exp)
{
	auto segment = previous_segment();

	//snapper: only entityset(not in snapper), singleton, collection(type or navprop) can have key
	bool haskey = false;
    ODATA_DEBUG_X("    try_bind_as_key_of_resource_collection for '" << identifier << "', parent type " << segment->segment_type()<< std::endl);
    if (segment->segment_type() == ::odata::core::odata_path_segment_type::Singleton)
		haskey = true;

    if (segment->segment_type() == ::odata::core::odata_path_segment_type::EntitySet)
		haskey = true;

    //Systems/1  ResourceCollection followed by key
	if (segment->segment_type() == ::odata::core::odata_path_segment_type::Type)
	{
		std::string basename;
		auto type_name = segment->as<odata_type_segment>()->type()->get_name();
		auto enttype = model()->find_entity_type(type_name);
		if (enttype != nullptr)
			basename = enttype->get_base_type_name();
		else{
			auto comptype = model()->find_complex_type(type_name);
			if(comptype != nullptr)
				basename = model()->find_complex_type(type_name)->get_base_type_name();
		}
		ODATA_DEBUG_X("    parent type " << type_name  << ", base name " << basename << std::endl);
		if(basename.find("ResourceCollection") != std::string::npos)
			haskey = true;
	}

    // autoexpand Systems/1/PCIeFunctions/ob_1, Collection(PCIeFunction.PCIeFunction) followed by key
    bool autoexpand = false;
	if (segment->segment_type() == ::odata::core::odata_path_segment_type::NavigationProperty) 
	{
		std::string nav_typename = segment->as<odata_navigation_property_segment>()->navigation_type()->get_name();
		if(nav_typename.find("Collection(") != std::string::npos && nav_typename.find(")") != std::string::npos)
		{
			haskey = true;
            autoexpand = true;
		}
		ODATA_DEBUG_X("    parent type " << nav_typename << std::endl);
	}

	if(haskey == false)
    {
    	ODATA_DEBUG_X("    " << identifier << " can't be a key" << std::endl);
        return false;
	}
    ...
    //determine the type of current key
	auto target_entity_type = ::odata::edm::edm_model_utility::get_entity_type(target_type());
    ODATA_DEBUG_X("    key's target type name: " << target_type()->get_name() << std::endl);
    ODATA_DEBUG_X("    key's target entity name: " << target_entity_type->get_name() << std::endl);
    if(::odata::edm::edm_model_utility::is_resource_collection(target_entity_type)) //Systems/1  ResourceCollection, add a key segment
    {
        // if it is a ResourceCollection, when we come here, it means the key is for the member entity type
        // we will create path segments like entity_type/key. where key is Id:url_value pair
        auto member_prop_type = ::odata::edm::edm_model_utility::get_property_type_from_name(target_entity_type, "Members");
        if (member_prop_type->get_type_kind() == ::odata::edm::edm_type_kind_t::Entity)
        {
            ODATA_DEBUG_X("        ResourceCollection, try_bind_as_key_of_resource_collection" << std::endl);
            ODATA_DEBUG_X("        property Members [" << member_prop_type->get_name() <<"] of " << target_entity_type->get_name() << std::endl);
            if (try_bind_as_type(member_prop_type->get_name(), empty_exp, true))
            {                
                //compute new target after bind as type
                compute_target_navigation_source_and_type();

                //snapper: string member id is quoted by \' in odata spec, otherwise it's regarded as identifier token
                //redfish spec needs no '' so snapper has to make it string literal by adding ''
                ::odata::utility::string_t memberid = identifier;
                if( identifier.at(0) != '\'' && identifier.at(identifier.length()-1) != '\'')
                    memberid = "'"+identifier+ "'";

                auto keys = parse_named_values(memberid); //snapper needs ''                    
                auto target_entity_type = ::odata::edm::edm_model_utility::get_entity_type(target_type());
                auto keys_from_model = target_entity_type->get_key_with_parents(); 
                ODATA_DEBUG_X("    new key's target type name after bind type: " << target_type()->get_name() << std::endl);
                ODATA_DEBUG_X("    new key's target entity name after bind type: " << target_entity_type->get_name() << std::endl);
				...
				//ODATA_DEBUG_X("new key .........." << keys[0].first << " : " << keys[0].second->to_string() << std::endl);
                m_bound_segments.push_back(odata_path_segment::create_key_segment(target_navigation_source(), target_entity_type, std::move(keys)));
				ODATA_DEBUG_X("=>create key seg: " << "[" << target_entity_type->get_name()<<"]:<" << target_entity_type->get_base_type_name()<<"> , " << keys_from_model[0] << std::endl);
				//target_navigation_source()->get_name);

            }
            else
            {
                ODATA_DEBUG_X("    bind key failed, target entity " << target_entity_type->get_name() << " bind as type failed  "<< std::endl);
                return false;
            }
        }
        else  
        {
			ODATA_DEBUG_X("    bind key failed, target entity " << target_entity_type->get_name() << "has invalid Members property  "<< std::endl);
			return false;
        }
    }
    else if(autoexpand == true)
    {
        ODATA_DEBUG_X("        Collection(" << target_entity_type->get_name() << "), try_bind_as_key_of_resource_collection for Auto Expanded" << std::endl);
        if (try_bind_as_type(target_entity_type->get_name(), empty_exp, true))
        {    
            //compute new target after bind as type
            compute_target_navigation_source_and_type();
            ...
            for (auto it = keys_from_model.begin(); it != keys_from_model.end(); ++it)
            {
				...
				//ODATA_DEBUG_X("model key .........." << *it << std::endl);  "Id"
            }
        ...
        {
            ODATA_DEBUG_X("    bind key failed for target entity " << target_entity_type->get_name() << " bind as type failed" << std::endl);
            return false;
        }
    }
    else
    {
		ODATA_DEBUG_X("    bind key failed for target entity " << target_entity_type->get_name()  << std::endl);
        return false;
    }
    return true;
}
...
bool odata_path_parser::try_bind_as_operation_import(const ::odata::utility::string_t &identifier, const ::odata::utility::string_t &parameters_exp)
{
	...
	ODATA_DEBUG_X("=>create operation_import prop seg for '" << identifier << "' [" << operation_import->get_name() << "] " << operation_import->get_operation_name() << std::endl);
...
bool odata_path_parser::try_bind_as_operation(const ::odata::utility::string_t &identifier, const ::odata::utility::string_t &parameters_exp)
{
	...
	// snapper, parameter validation will be done in request_handle
	//validate_parameters_match(parameters, parameters_from_model, /*has_binding_parameter*/true);
	ODATA_DEBUG_X("=>create operation prop seg for '" << identifier << "' [" << operation->get_name() << "]" << std::endl);
...
void odata_path_parser::bind_as_dynamic_property(const ::odata::utility::string_t &identifier, const ::odata::utility::string_t &empty_exp)
{
	if (previous_segment()->segment_type() != ::odata::core::odata_path_segment_type::DynamicProperty)
	{
		...
		ODATA_DEBUG_X("   bind dynamic prop seg for '" << identifier << "' [" << target_structured_type->get_name() << "]: <" << target_structured_type->get_base_type_name() << ">" << std::endl);
		if (!empty_exp.empty())
		{
	...
	ODATA_DEBUG_X("=>create dynamic prop seg for '" << identifier << std::endl);
...
void odata_path_parser::compute_target_navigation_source_and_type()
{
    ...
    {
        ODATA_DEBUG_X("compute_target_navigation_source_and_type is not null  " << std::endl);             
...
std::shared_ptr<::odata::core::odata_uri> odata_uri_parser::parse_uri(const ::odata::utility::uri &uri)
{
    ...
    ODATA_DEBUG_V(uri.to_string());

	auto query_options = split_query_options(uri.query());
    ODATA_DEBUG_V(uri.query());
