...
namespace odata { namespace edm {
    ...
    bool xml_reader::parse()
    {
        ...
        while (m_continueParsing && xmlTextReaderRead(m_reader))
        {
...
#endif
                auto elem = handle_begin_element(name);
                m_elementStack.push_back(elem);

                if (is_empty_element)
                {
                    auto n = m_elementStack.back();
            ...
            {
                auto n = m_elementStack.back();
                handle_end_element(m_elementStack.back()->name);
                m_elementStack.pop_back();
                if(n!=nullptr)
                    n->reset();
                break;
    ...
    string_t xml_reader::get_parent_element_name(size_t pos)
    {
        if (m_elementStack.size() > pos + 1)
        {
            ...
            if (pos <= parentDepth)
            {
                return m_elementStack[parentDepth - pos]->name;
            }
        }
    ...
    //snapper
    std::shared_ptr<xml_elem> xml_reader::get_parent_element(size_t pos)
    { // last elem is itself, pos is 0 based, pos = 0 is direct parent.
      // should be used after handle_begin_element() which will push_back new node
        if (m_elementStack.size() > pos + 1)
        {
            size_t currentDepth = m_elementStack.size() - 1;
            size_t parentDepth = currentDepth - 1;
    ...
    std::shared_ptr<xml_elem> xml_reader::get_last_element()
    {
