...
void edm_model_writer::write_schema(std::shared_ptr<edm_schema> schema)
{
	...
	//snapper
    for(auto iter = schema->get_type_defs().cbegin(); iter != schema->get_type_defs().cend(); iter++)
    {
        write_type_def(iter->second);
    }
...
//snapper
void edm_model_writer::write_type_def(std::shared_ptr<edm_type_def> type_def)
{
    write_start_element(U("TypeDefinition"));
    write_attribute_string(U(""), U("Name"), U(""), type_def->get_name());
