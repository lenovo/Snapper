...
{    
    ...
    //{
    //    std::cout << "find_property" << it->first << std::endl;
    //}
...
std::shared_ptr<edm_enum_member> edm_enum_type::find_enum_member(::odata::utility::string_t member_name) const
{
    for(int i = 0; i < m_members.size(); i++)
    {
        if(m_members[i]->get_enum_member_name() == member_name)
            return m_members[i];
    }
