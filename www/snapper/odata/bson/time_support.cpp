...
#include <string>
#include "bson/util/json_free_inl.h"
#include "bson/util/json.h"
#include "bson/util/time_support.h"
...
namespace bson {
    ...
    StringData getNextToken(const StringData& currentString,
        ...
        for (; index < currentString.size(); index++) {
            if (terminalChars.find(currentString[index], 0) != std::string::npos) {
                break;
            }
        }
    ...
    bool isOnlyDigits(const StringData& toCheck) {
        ...
        for (StringData::const_iterator iterator = toCheck.begin();
            ...
            if (digits.find(*iterator, 0) == std::string::npos) {
                return false;
            }
        }
        return true;
