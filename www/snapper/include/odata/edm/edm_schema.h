...
public:
    ...
    /// 
    /// </summary>
    void add_type_def(std::shared_ptr<edm_type_def> td)//snapper
    {
        m_type_defs[td->get_name()] = td;
    }
	...
	//snapper
	const std::unordered_map<::odata::utility::string_t, std::shared_ptr<edm_type_def>>& get_type_defs() const
    {
        return m_type_defs;
    }
	...
	//snapper
	std::vector<std::shared_ptr<edm_type_def>> get_type_defs_vector() const
	{
		std::vector<std::shared_ptr<edm_type_def>> ret;
		for (auto it = m_type_defs.cbegin(); it != m_type_defs.cend(); ++it)
			ret.push_back(it->second);
		return ret;
	}
...
private:
	...
	std::unordered_map<::odata::utility::string_t, std::shared_ptr<edm_type_def>> m_type_defs;//snapper
